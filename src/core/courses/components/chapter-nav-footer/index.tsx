...
interface ChapterNavFooterProps {
  ...
  nextSectionText?: string | null
  nextChapter?: NextChapterData | null
  courseHref: string
}

const ChapterNavFooter = ({
  quiz,
  nextSectionText,
  nextChapter,
  courseHref,
  ...
  const showNextLink = !hasQuiz || quizPassed
    ...
    <div className="page-content-block space-y-12">
      {hasQuiz ? (
        <PopQuiz
          ...
          nextSectionDescription={nextSectionText ?? ''}
      ...
      {showNextLink ? (
        <div className="max-w-screen-sm space-y-4">
          ...
          {nextSectionText && hasQuiz ? (
            <Typography className="text-body">{nextSectionText}</Typography>
          ) : null}
