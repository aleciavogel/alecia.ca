...
module.exports = (
  ...
  visit(markdownAST, `code`, (node) => {
    ...
    const match = splitLanguage ? splitLanguage.match(DIFF_HIGHLIGHT_SYNTAX) : null;
    ...
    const className = `${classPrefix}${languageName}${diffLanguage ? `-${diffLanguage}` : ``}`;
    ...
    +   `${codeTitle ? `<div class="gatsby-code-title">${codeTitle}</div>` : ``}`
  ...
  if (!noInlineHighlight) {
    visit(markdownAST, `inlineCode`, (node) => {
      ...
      if (inlineCodeMarker) {
        const [language, restOfValue] = node.value.split(`${inlineCodeMarker}`, 2);
